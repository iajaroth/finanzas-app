...
function computeBalances() {
  ...
  const toAgg = db.prepare(`
    SELECT transfer_to_id AS id, SUM(amount) AS transferred FROM transactions
    WHERE type = 'transfer' AND transfer_to_id IS NOT NULL GROUP BY transfer_to_id`).all();
  const fromAgg = db.prepare(`
    SELECT account_id AS id, SUM(amount) AS transferred FROM transactions
...
export function apiRouter() {
  ...
  r.post('/transactions', (req, res) => {
    ...
    const info = db.prepare(
      'INSERT INTO transactions (account_id, transfer_to_id, category_id, type, amount, merchant, description, notes, occurred_at, source) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
    ).run(b.account_id || null, b.transfer_to_id || null, b.type === 'transfer' ? null : (b.category_id || null),
      b.type, Math.round(Number(b.amount)), b.merchant || '', b.description || '', b.notes || '', b.occurred_at, b.source || 'manual');
    res.json(txById(info.lastInsertRowid));
  });
  r.put('/transactions/:id', (req, res) => {
    ...
    if (err) return res.status(400).json({ error: err });
    db.prepare(
      `UPDATE transactions SET account_id = ?, transfer_to_id = ?, category_id = ?, type = ?, amount = ?,
       merchant = ?, description = ?, notes = ?, occurred_at = ?, updated_at = datetime('now') WHERE id = ?`
    ).run(b.account_id || null, b.transfer_to_id || null, b.type === 'transfer' ? null : (b.category_id || null),
      b.type, Math.round(Number(b.amount)), b.merchant || '', b.description || '', b.notes || '', b.occurred_at, t.id);
  ...
  r.get('/stats', (req, res) => {
    ...
    const { start, end } = monthRange(month);
    const byCategory = db.prepare(`
      SELECT c.id AS category_id, c.name, c.icon, c.color, SUM(t.amount) AS total, COUNT(*) AS n
      FROM transactions t JOIN categories c ON c.id = t.category_id
      WHERE t.type = 'expense' AND t.occurred_at >= ? AND t.occurred_at < ?
      GROUP BY c.id ORDER BY total DESC`).all(start, end);
    const byAccount = db.prepare(`
      SELECT a.id AS account_id, a.name, a.kind, a.color, COALESCE(a.bank, '') AS bank, SUM(t.amount) AS total
      FROM transactions t JOIN accounts a ON a.id = t.account_id
      WHERE t.type = 'expense' AND t.occurred_at >= ? AND t.occurred_at < ?
      GROUP BY a.id ORDER BY total DESC`).all(start, end);
    const topMerchants = db.prepare(`
      SELECT COALESCE(NULLIF(t.merchant, ''), t.description, 'Sin detalle') AS merchant, SUM(t.amount) AS total, COUNT(*) AS n
      FROM transactions t
      WHERE t.type = 'expense' AND t.occurred_at >= ? AND t.occurred_at < ?
    ...
    for (const msg of messages) {
      ...
      if (auto) {
        const txInfo = db.prepare(
          'INSERT INTO transactions (account_id, category_id, type, amount, merchant, description, occurred_at, source) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
        ).run(acc?.id || null, categoryId, parsed.type, parsed.amount, parsed.merchant,
  ...
  r.patch('/email/imports/:id', (req, res) => {
  ...
  r.post('/email/imports/approve-all', (req, res) => {
    ...
    for (const imp of pending) {
      const info = db.prepare(
        'INSERT INTO transactions (account_id, category_id, type, amount, merchant, description, occurred_at, source) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
      ).run(imp.account_id, imp.category_id, imp.type, imp.amount, imp.merchant,
